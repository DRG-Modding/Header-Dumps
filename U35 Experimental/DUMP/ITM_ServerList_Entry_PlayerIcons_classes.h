...
struct UITM_ServerList_Entry_PlayerIcons_C : UUserWidget {
	...
	TArray<UPlayerCharacterID*> Players; // 0x280(0x10)
	bool ClassLocked; // 0x290(0x01)
	TArray<UImage*> PlayerIcons; // 0x298(0x10)
	int32_t MaxIcons; // 0x2a8(0x04)

	void Get Player Details(TArray<UPlayerCharacterID*> Players, int32_t PlayerIndex, bool IsClassLocked, FLinearColor OutColor, UTexture2D* OutIcon, UPlayerCharacterID* OutPlayer);
	UTOOLTIP_ServerEntry_Team_C* GetToolTip();
	void SetIndexBrush(int32_t NumPlayers, int32_t EntryNum, UImage* Target, TArray<UPlayerCharacterID*> Players, bool IsClassLocked);
	void SetNumPlayers(int32_t NumPlayers, TArray<UPlayerCharacterID*> Players, bool IsClassLocked);
	void Construct();
