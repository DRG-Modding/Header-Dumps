// BlueprintGeneratedClass WPN_CombatShotgun.WPN_CombatShotgun_C
// Size: 0x6d0 (Inherited: 0x698)
struct AWPN_CombatShotgun_C : AAutoShotgun {
	FPointerToUberGraphFrame UberGraphFrame; // 0x698(0x08)
	UFirstPersonStaticMeshComponent* FirstPersonStaticMesh; // 0x6a0(0x08)
	UFirstPersonWidgetComponent* FirstPersonWidget; // 0x6a8(0x08)
	UDamageComponent* Damage; // 0x6b0(0x08)
	UPointLightComponent* MuzzlePointLight; // 0x6b8(0x08)
	UCrosshairAggregator* CrosshairAggregator; // 0x6c0(0x08)
	UMultiHitscanComponent* MultiHitscan; // 0x6c8(0x08)

	void GetGearStatEntry(AFSDPlayerState* PlayerState, TArray<FGearStatEntry> Stats);
	void UserConstructionScript();
	void ReceiveBeginPlay();
	void ExecuteUbergraph_WPN_CombatShotgun(int32_t EntryPoint);
};

